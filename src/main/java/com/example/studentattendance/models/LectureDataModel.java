            ...
            if (file.exists()) {
                try (Scanner scanner = new Scanner(file)) {
                    ...
                    while (scanner.hasNext()) {
                        ...
                        tempLectures.add(new Lecture(Integer.parseInt(strings[0]),Integer.parseInt(strings[1]),strings[2],strings[3],strings[4]));
                    }
    ...
    public void saveLecture() {
        try (PrintWriter pw = new PrintWriter("lecture.csv")) {
            ...
            for (Lecture lecture : lectures) {
                pw.println(lecture.getLCode() + "," +lecture.getTeacher_Code() +","+lecture.getTeacher_name() + "," + lecture.getLName() + "," + lecture.getClassroom());
            }
        } catch (FileNotFoundException e) {
            e.printStackTrace();
        }
